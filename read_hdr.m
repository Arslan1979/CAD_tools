function [stack_all,inf]=read_hdr(directorio,VS,varargin)
%
...
%brain=dir([directorio '\normalizados\*.hdr']);fprintf('.')
cd(directorio)
brain=dir([directorio '/*.hdr']);fprintf('.')
bnames=dir([directorio '/*.hdr']);fprintf('.')
bnamxml=dir([directorio '/*.xml']);fprintf('.')
%bimg=dir([directorio '\normalizados\*.img']);fprintf('.')
bimg=dir([directorio '/*.img']);fprintf('.')
...
%%
    ...
    info=analyze75info(brain(i).name);fprintf('.')
    stack=analyze75read(info);fprintf('.')
%     delete(nbrain)
%     delete(ibrain)
%    stack=double(stack);
    if ~isempty(varargin)
