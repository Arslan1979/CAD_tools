function [X,Y,auc,output,test]=roc(data,labels,step)
...
for nk=1:numel(kernels)
    ...
    tdata=data(train,:);
    svmStruct = svmtrain(tdata,labels(train),'Kernel_Function',char(kernels(nk)));
    ...
    for i=min_val:(max_val-min_val)/step:max_val
        if all(svmStruct.Alpha>0==tt_labels(svmStruct.SupportVectorIndices)')
        classes=int8(output>i)';
        elseif all(svmStruct.Alpha<0==tt_labels(svmStruct.SupportVectorIndices)')
            classes=int8(output<i)';
        else
            disp('Raro!')
        end
        cp=classperf(labels(test),classes,'Positive',1,'Negative',0);
